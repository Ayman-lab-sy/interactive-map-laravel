...
use TCG\Voyager\Http\Controllers\Controller as VBController;
class VController extends VBController
{
    ...
    public function upload(Request $request)
    {
        ...
        } else if (in_array($ext, ['mp4', 'webm', '3gp'])) {
            $vFile = Storage::disk(config('voyager.storage.disk'))->put($fullPath, file_get_contents($file), 'public');
            if ($vFile) {
                $status = __('voyager::media.success_uploading');
                $fullFilename = $fullPath;
            } else {
    ...
    protected function userCannotUploadImageIn($dataType, $action)
    {
        return auth()->user()->cannot($action, app($dataType->model_name))
                || ($dataType->{$action.'Rows'}->where('type', 'tiny mce full')->count() === 0 && $dataType->slug !== 'site-settings');
    }
}
