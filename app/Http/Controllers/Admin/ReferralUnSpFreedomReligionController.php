...
class ReferralUnSpFreedomReligionController extends Controller
{
    public function show(int $referralId)
    {
        ...
        $steps = (object)[
            ...
            'identity_saved'     => !empty($data?->minority_or_religious_identity_en),
            'violation_saved'    => !empty($data?->violation_description_en),
        ...
        return view(
            'admin.referrals.show_un_special_freedom_religion',
    ...
    public function saveSummary(Request $request, int $referralId)
    {
        ...
        $this->saveField(
            ...
            'un_sp_freedom_religion_summary_saved'
        );

        return redirect()->route(
            'admin.referrals.un_sp.freedom_religion.show',
            $referralId
    ...
    public function saveVictim(Request $request, int $referralId)
    {
        ...
        $this->saveField(
            ...
            'un_sp_freedom_religion_victim_saved'
        );
    ...
    public function saveIdentity(Request $request, int $referralId)
    {
        $this->assertEditable($referralId);

        $request->validate([
            'minority_or_religious_identity_en' => 'required|string',
        ]);

        $this->saveField(
            $referralId,
            ['minority_or_religious_identity_en' => $request->minority_or_religious_identity_en],
            'un_sp_freedom_religion_identity_saved'
        );

        return redirect()->route(
            'admin.referrals.un_sp.freedom_religion.show',
            $referralId
        )->with('success', 'تم حفظ الهوية الدينية.')
         ->withFragment('block-d');
    }

    public function saveViolation(Request $request, int $referralId)
    {
        $this->assertEditable($referralId);

        $request->validate([
            'violation_description_en' => 'required|string',
        ]);

        $this->saveField(
            $referralId,
            ['violation_description_en' => $request->violation_description_en],
            'un_sp_freedom_religion_violation_saved'
        );

        return redirect()->route(
            'admin.referrals.un_sp.freedom_religion.show',
            $referralId
        )->with('success', 'تم حفظ وصف الانتهاكات.')
    ...
    public function savePerpetrators(Request $request, int $referralId)
    {
        ...
        $this->saveField(
            ...
            'un_sp_freedom_religion_perpetrators_saved'
        );
    ...
    public function saveContext(Request $request, int $referralId)
    {
        ...
        $this->saveField(
            ...
            'un_sp_freedom_religion_context_saved'
        );
    ...
    public function skipContext(int $referralId)
    {
        ...
        DB::connection('cases')->table('case_referral_audits')->insert([
            'referral_id' => $referralId,
            'action' => 'un_sp_freedom_religion_context_skipped',
            'user_id' => auth()->id(),
            'created_at' => now(),
        ]);
    ...
    public function saveRemedies(Request $request, int $referralId)
    {
        ...
        $this->saveField(
            ...
            'un_sp_freedom_religion_remedies_saved'
        );
