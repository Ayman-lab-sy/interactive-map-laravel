...
class ReferralUnSpFreedomExpressionController extends Controller
{
    public function show(int $referralId)
    {
        ...
        $steps = (object)[
            ...
            'expression_saved' => !empty($data?->expression_activity_en),
        ...
        return view(
            'admin.referrals.show_un_special_freedom_expression',
    ...
    public function saveSummary(Request $request, int $referralId)
    {
        ...
        $this->saveField(
            ...
            'un_sp_fe_summary_saved'
        );

        return redirect()
            ->route('admin.referrals.un_sp.freedom_expression.show', $referralId)
    ...
    public function saveExpressionActivity(Request $request, int $referralId)
    {
        $this->assertEditable($referralId);

        $request->validate([
            'expression_activity_en' => 'required|string',
        ]);

        $this->saveField(
            $referralId,
            ['expression_activity_en' => $request->expression_activity_en],
            'un_sp_fe_expression_saved'
        );

        return redirect()
            ->route('admin.referrals.un_sp.freedom_expression.show', $referralId)
            ->with('success', 'تم حفظ وصف الرأي أو النشاط التعبيري بنجاح')
    ...
    public function saveViolations(Request $request, int $referralId)
    {
        ...
        $this->saveField(
            ...
            'un_sp_fe_violations_saved'
        );

        return redirect()
            ->route('admin.referrals.un_sp.freedom_expression.show', $referralId)
            ->with('success', 'تم حفظ تفاصيل الانتهاكات أو القمع بنجاح')
    ...
    public function saveLegalBasis(Request $request, int $referralId)
    {
        ...
        $this->saveField(
            ...
            'un_sp_fe_legal_basis_saved'
        );

        return redirect()
            ->route('admin.referrals.un_sp.freedom_expression.show', $referralId)
            ->with('success', 'تم حفظ الأساس القانوني بنجاح')
    ...
    public function skipContext(int $referralId)
    {
        ...
        DB::connection('cases')->table('case_referral_audits')->insert([
            'referral_id' => $referralId,
            'action' => 'un_sp_fe_context_skipped',
            'user_id' => auth()->id(),
            'created_at' => now(),
        ]);
